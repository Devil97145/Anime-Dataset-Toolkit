...
class ImageTagEditor:
    def __init__(self):
        ...
        self.histories = {}        # {txt_path: history list}
        self.history_indices = {}  # {txt_path: index}
    ...
    def batch_delete_tags(self, folder_path, tag_to_delete, use_regex=False):
        ...
        for txt_file in txt_files:
            try:
                ...
                if use_regex:
                    pattern = re.compile(tag_to_delete)
    ...
    def batch_append_tags(self, folder_path, tags_to_append, position="end"):
        ...
        if not txt_files:
            return "⚠️ 未找到任何 .txt 文件"
        append_list = [t.strip() for t in tags_to_append.split(',') if t.strip()]
        if not append_list:
            return "❌ 追加标签为空"
        count = 0
        for txt_file in txt_files:
