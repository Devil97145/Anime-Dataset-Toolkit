...
class VideoFrameExtractor:
    def __init__(self, input_dir, output_dir, frame_interval=90, max_threads=4, use_gpu=False):
        """
        初始化视频帧提取器
        """
        ...
        print(f"[初始化] 🖥️ 硬件加速: {'GPU (CUDA)' if use_gpu else 'CPU'}", flush=True)
    ...
    @staticmethod
    def extract_single_video(video_path, output_dir, frame_interval=90, use_gpu=False):
        """
        静态方法：处理单个视频文件
        """
